...
public class DistributedLexBIGServiceConvenienceMethodsImplTest extends LexBIGServiceTestCase {
    ...
    public void setUp() throws LBException{
        lbs = LexEVSServiceHolder.instance().getAssertedValueSetConfiguredLexEVSAppService(
...
//			set = set.restrictToMatchingDesignations("Patient", SearchDesignationOption.ALL, "LuceneQuery", null);
    ...
    public void searchAllDecendentsInTransitiveClosureDomainMildlySickPatientSourceSpecificPreferred( ) throws LBParameterException{
    	...
    	assertTrue(Arrays.asList(refs.getResolvedConceptReference()).stream().anyMatch(x -> x.getCode().equals("PatientWithCold")));
    	assertTrue(Arrays.asList(refs.getResolvedConceptReference()).stream().anyMatch(x -> x.getCodeNamespace().equals("owl2lexevs")));
    	assertTrue(Arrays.asList(refs.getResolvedConceptReference()).stream().anyMatch(x -> x.getEntityDescription().getContent().equals("Patient With Cold"))); 
    }
    ...
    @Test
    public void getTerminologyServiceDesignationForUri() throws LBException{
        lbs = LexEVSServiceHolder.instance().getAssertedValueSetConfiguredLexEVSAppService(
        		new AssertedValueSetParameters.Builder("0.1.5").
		assertedDefaultHierarchyVSRelation("Concept_In_Subset").
		codingSchemeName("owl2lexevs").
        ...
        lbscm = (LexBIGServiceConvenienceMethodsImpl)lbs.getGenericExtension("LexBIGServiceConvenienceMethods");
        lbscm.setLexBIGService(lbs);
    	assertEquals(lbscm.getTerminologyServiceObjectType(AUTO_URN).getDesignation(), 
    			new TerminologyServiceDesignation(
    			TerminologyServiceDesignation.REGULAR_CODING_SCHEME).getDesignation());
    	assertEquals(lbscm.getTerminologyServiceObjectType(MAPPING_SCHEME_URI).getDesignation(), 
    			new TerminologyServiceDesignation(
    					TerminologyServiceDesignation.MAPPING_CODING_SCHEME).getDesignation());
    	assertEquals(lbscm.getTerminologyServiceObjectType("SRITEST:AUTO:AllDomesticButGM").getDesignation(), 
    			new TerminologyServiceDesignation(
    					TerminologyServiceDesignation.RESOLVED_VALUESET_CODING_SCHEME).getDesignation());
