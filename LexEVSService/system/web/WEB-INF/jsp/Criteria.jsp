...
if(className != null)
{
	try
	{	
		jspUtils = LexEVSJSPUtils.getJSPUtils(config.getServletContext());
...
-->		
		<% CodingSchemeRendering[] codingSchemes = jspUtils.getLoadedCodingSchemes(); 
		String codingSchemeName = "";
		String codingSchemeVersion = "";
		%>
		<tr>
			<td class="formRequiredNotice" width="5px">&nbsp;</td>
			<td class="formRequiredLabel" align="right">Coding Scheme:</td>
			<td class="formField" width="90%"><SELECT class="formFieldSized" NAME="selectedCodingScheme">
					<OPTION SELECTED value="">All Coding Schemes</OPTION>
			   		<% 
			   		
			   		for(int i=0; i<codingSchemes.length; i++){
			   			codingSchemeName = codingSchemes[i].getCodingSchemeSummary().getLocalName();
			   			codingSchemeVersion = codingSchemes[i].getCodingSchemeSummary().getRepresentsVersion();
			   		%>	<OPTION value=<%=i%>> <%=codingSchemeName%> - <%=codingSchemeVersion%></OPTION> <%
			   		}
					%>
					</SELECT>
			</td>
